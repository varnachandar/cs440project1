...
def makeGrid(filename):
    ...
    f.close()
    return [startx,starty,endx,endy,blocked,gval,parent,dim1, dim2]

# general array method:
def findIndex (x,y,dim2): # gives the index for the arrays for gval,blocked and parent
    return ((x-1)*dim2) + y -1
...
def createPath(parent,startx,starty,endx,endy,path,dim2):
    index = findIndex(endx,endy,dim2)
    [px,py] = parent[index]
    path.append([endx,endy])
    counter = 0
    while counter != 2: 
        path.append(parent[index])
        index = findIndex(px,py,dim2)
        [px,py] = parent[index]
...
# update vertex:
def UpdateVertex(a,b, c, d, gval,endx,endy,dim2): 
    index_s = findIndex(a,b,dim2)
    index_sprime = findIndex(c,d,dim2)
    cval = cfunc(a,b,c,d)
...
# A*
def A_star(a,b,c,d,blocked,gval,parent,dim1,dim2):
    startx = a
    starty = b
    endx = c
    endy = d
    indexS = findIndex(startx,starty,dim2)
    gval[indexS] = 0
    ...
    while len(fringe) != 0:
        ...
        successor = []
        successor = findSuccessor(x,y,blocked,successor,dim1,dim2) 
        ...
        while len(successor) != 0:
            ...
            if not cElementOf(sx, sy):
                if not elementOf(sx,sy):
                    ...
                    gval[si] = float('inf')
                    parent[si] = None 
                UpdateVertex(x,y,sx,sy,gval,endx,endy,dim2)
...
def LineOfSight(a,b,c,d,blocked,dim2):
    x0 = a
    y0 = b
    x1 = c
    y1 = d
    ...
    if dx >= dy:
        while x0 != x1:
            f = f+dy
            if f >= dx:
                if blocked[findIndex(x0+ s_x, y0+s_y,dim2)] == 1:
                    return False
                y0 = y0 + dod_y
                f = f-dx
            if f != 0 and blocked[findIndex(x0+s_x, y0+s_y,dim2)] == 1:
                return False
            if dy == 0 and blocked[findIndex(x0+s_x, y0,dim2)] == 1 and gval[findIndex(x0+s_x, y0-1,dim2)] == 1:
                return False
            x0 = x0 + dod_x
    else:
        while y0 != y1:
            ...
            if f != 0 and blocked[findIndex(x0+s_x, y0+s_y,dim2)] == 1:
                return False
            if dy == 0 and blocked[findIndex(x0, y0+s_y,dim2)] == 1 and blocked[findIndex(x0-1, y0 +s_y,dim2)] == 1:
                return False
            y0 = y0 + dod_y
    return True

def thetaUpdateVertex(px,py,x,y,sx,sy,gval,parent,endx,endy,blocked,dim2): 
    index_p = findIndex(px,py,dim2)
    index_sprime = findIndex(sx,sy,dim2)
...
def theta(startx,starty,endx,endy,blocked,gval,parent,dim1,dim2):
    indexS = findIndex(startx,starty,dim2)
    gval[indexS] = 0
    parent[indexS] = [startx,starty]
...
def main():
    ...
    [startx,starty,endx,endy,blocked,gval,parent,dim1,dim2] = makeGrid(filename)
    if (method == "A"):
        A_star(startx,starty,endx,endy,blocked,gval,parent,dim1,dim2)
    elif(method == "Theta"):
        theta(startx,starty,endx,endy,blocked,gval,parent,dim1,dim2)
