...
fringe = [] # open list
closed = set() # closed set (was a list before)
...
# code for visibility graph
def visibilityGraph(startx,starty,endx,endy,blocked):
    # draw the vertex for start and end
    # just use the same window size from before no point trying to shift or wtvr so you can get to 0 or smtg
    visibility = tk.Tk()
    visibilityCanvas = tk.Canvas(visibility, bg='white', height=1700, width=1700) #using the 1000 1000 gives a height of 50 and width of 100 tbh idk why
    visibilityCanvas.pack(fill=tk.BOTH, expand=True)
    
    if (startx > endx) and (starty > endy):
        for i in range (endx, startx):
            for j in range (endy, starty):
                if (blocked[findIndex(i,j)] == 1):
                    visibilityCanvas.create_rectangle(i*17, j*17, (i*17)+17, (j*17)+17, outline = 'white', fill='gray')
                    # draw the square so that it is gray inside and then white boundary only for blocked
                    visibilityCanvas.create_oval((i*17)-2, (j*17)-2, (i*17)+2, (j*17)+2, outline = 'black', fill='black')
                    visibilityCanvas.create_oval((i*17)-2, ((j+1)*17)-2, (i*17)+2, ((j+1)*17)+2, outline = 'black', fill='black')
                    visibilityCanvas.create_oval(((i+1)*17)-2, (j*17)-2, ((i+1)*17)+2, (j*17)+2, outline = 'black', fill='black')
                    visibilityCanvas.create_oval(((i+1)*17)-2, ((j+1)*17)-2, ((i+1)*17)+2, ((j+1)*17)+2, outline = 'black', fill='black')
                    # draw the vertices
    ...
    visibilityCanvas.create_oval((startx*17)-3, (starty*17)-3, (startx*17)+3, (starty*17)+3, outline = 'black', fill='red') 
    visibilityCanvas.create_oval((endx*17)-3, (endy*17)-3, (endx*17)+3, (endy*17)+3, outline = 'black', fill='green')
...
def main():
    ...
    visibilityGraph(startx,starty,endx,endy,blocked)
